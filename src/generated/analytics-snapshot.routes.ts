...
/**
 * Auto-generated AnalyticsSnapshot Routes
 *
...
import type { AnalyticsSnapshotHooks } from "./analytics-snapshot.hooks"
...
let hooks: AnalyticsSnapshotHooks = {}
...
/**
 * Set hooks for AnalyticsSnapshot routes
 */
export function setAnalyticsSnapshotHooks(h: AnalyticsSnapshotHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["date", "subscribers", "totalViews", "watchTimeHours", "avgViewDurationMins", "topVideo", "notes", "createdAt"] as const
...
/**
 * Create AnalyticsSnapshot CRUD routes
 */
export function createAnalyticsSnapshotRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[AnalyticsSnapshot] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.analyticsSnapshot.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "AnalyticsSnapshot not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[AnalyticsSnapshot] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      console.error("[AnalyticsSnapshot] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.analyticsSnapshot.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[AnalyticsSnapshot] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[AnalyticsSnapshot] Delete error:", error)
