...
/**
 * Auto-generated VideoIdea Routes
 *
...
import type { VideoIdeaHooks } from "./video-idea.hooks"
...
/**
 * Set hooks for VideoIdea routes
 */
export function setVideoIdeaHooks(h: VideoIdeaHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["title", "description", "category", "priority", "estimatedEffort", "status", "createdAt", "updatedAt"] as const
...
/**
 * Create VideoIdea CRUD routes
 */
export function createVideoIdeaRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.videoIdea.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[VideoIdea] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.videoIdea.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "VideoIdea not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[VideoIdea] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.videoIdea.create({
        data: picked.data,
      })
      ...
      console.error("[VideoIdea] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.videoIdea.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[VideoIdea] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[VideoIdea] Delete error:", error)
