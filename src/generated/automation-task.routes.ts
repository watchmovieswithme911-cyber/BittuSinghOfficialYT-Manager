...
/**
 * Auto-generated AutomationTask Routes
 *
...
import type { AutomationTaskHooks } from "./automation-task.hooks"
...
/**
 * Set hooks for AutomationTask routes
 */
export function setAutomationTaskHooks(h: AutomationTaskHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["type", "title", "description", "genre", "language", "status", "priority", "result", "error", "runsCount", "lastRunAt", "nextRunAt", "isRecurring", "intervalMins", "createdAt", "updatedAt"] as const
...
/**
 * Create AutomationTask CRUD routes
 */
export function createAutomationTaskRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[AutomationTask] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "AutomationTask not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[AutomationTask] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.automationTask.create({
        data: picked.data,
      })
      ...
      console.error("[AutomationTask] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      console.error("[AutomationTask] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[AutomationTask] Delete error:", error)
