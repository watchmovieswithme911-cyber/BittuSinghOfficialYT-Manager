...
import type { VideoHooks } from "./video.hooks"
...
/**
 * Set hooks for Video routes
 */
export function setVideoHooks(h: VideoHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["title", "description", "status", "tags", "scheduledDate", "publishedDate", "youtubeUrl", "thumbnailUrl", "views", "likes", "notes", "createdAt", "updatedAt"] as const
...
export function createVideoRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Video not found" } }, 404)
      }
