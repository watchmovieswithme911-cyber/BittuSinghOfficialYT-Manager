...
import type { GenreHooks } from "./genre.hooks"
...
/**
 * Set hooks for Genre routes
 */
export function setGenreHooks(h: GenreHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "slug", "language", "description", "bpmRange", "mood", "tags", "iconEmoji", "colorHex", "videoCount", "avgViews", "isActive", "createdAt", "updatedAt"] as const
...
/**
 * Create Genre CRUD routes
 */
export function createGenreRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Genre not found" } }, 404)
      }
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.genre.create({
        data: picked.data,
      })
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.genre.update({
        where: { id },
        data: picked.data,
      })
