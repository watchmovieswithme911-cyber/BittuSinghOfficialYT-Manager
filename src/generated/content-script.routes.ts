...
/**
 * Auto-generated ContentScript Routes
 *
...
import type { ContentScriptHooks } from "./content-script.hooks"
...
/**
 * Set hooks for ContentScript routes
 */
export function setContentScriptHooks(h: ContentScriptHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["videoTitle", "videoType", "genre", "language", "content", "tags", "duration", "mood", "status", "createdAt", "updatedAt"] as const
...
/**
 * Create ContentScript CRUD routes
 */
export function createContentScriptRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.contentScript.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[ContentScript] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.contentScript.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "ContentScript not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[ContentScript] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.contentScript.create({
        data: picked.data,
      })
      ...
      console.error("[ContentScript] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.contentScript.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[ContentScript] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      await prisma.contentScript.delete({
        where: { id },
      })
      ...
      console.error("[ContentScript] Delete error:", error)
