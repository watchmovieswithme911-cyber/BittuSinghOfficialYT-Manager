...
/**
 * Auto-generated UploadQueue Routes
 *
...
import type { UploadQueueHooks } from "./upload-queue.hooks"
...
/**
 * Set hooks for UploadQueue routes
 */
export function setUploadQueueHooks(h: UploadQueueHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["videoTitle", "genre", "language", "videoType", "status", "scheduledTime", "platform", "thumbnailUrl", "youtubeUrl", "views", "likes", "notes", "createdAt", "updatedAt"] as const
...
/**
 * Create UploadQueue CRUD routes
 */
export function createUploadQueueRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[UploadQueue] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.uploadQueue.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "UploadQueue not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[UploadQueue] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      console.error("[UploadQueue] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.uploadQueue.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[UploadQueue] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[UploadQueue] Delete error:", error)
